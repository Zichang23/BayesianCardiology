...
#     Data Exploration    
############################
# #create variable expect number of death
# library(dplyr)
# #age-standardized rate for Texas from 2011 to 2020 (CDC Wonder)
# Mean.Rate <- 169.9
# #calculate the expected number of heart disease death
# df3 <- df2 %>% group_by(County) %>% mutate(Expect = Mean.Rate*Pop/100000)
# df3 <- df3[,c(1:4,7:10)]
# 
# #calculate standardized mortality rate (SMR)
# df3 <- df3 %>% mutate(SMR=Deaths/Expect)
